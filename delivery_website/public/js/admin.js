...
function syncGalleryFileInput() {
  const galleryImageInput = document.getElementById(
    "product-gallery-image-input",
  );
...
function getGalleryPreviewItems() {
  const existingGalleryImages = currentEditingProductImages
    .slice(1)
    .map((src, index) => ({
      src,
      source: "existing",
...
document.addEventListener("DOMContentLoaded", () => {
        ...
        if (targetId === "add-product") {
          ...
          if (!editingProductId) {
            ...
            document
              ...
              .querySelector("h1").textContent = "Add Product";
  ...
  const openCustomerModalBtn = document.getElementById(
    "open-customer-modal-btn",
  );
  ...
  if (openCustomerModalBtn && customerModal) {
    openCustomerModalBtn.addEventListener("click", () => {
      document
        ...
        .querySelector("h2").textContent = "Add New Customer";
      document
        .getElementById("add-customer-modal")
        .querySelector(".btn-primary").textContent = "Save Customer";
    ...
    if (addCustomerForm) {
      addCustomerForm.addEventListener("submit", (e) => {
        e.preventDefault();
        showToast(
          "Customer feature is view-only (managed via user registration)",
        );
        customerModal.style.display = "none";
        addCustomerForm.reset();
      });
  ...
  if (addProductForm) {
    addProductForm.addEventListener("submit", async function (e) {
      ...
      if (cat === "__other__") {
        ...
        try {
          const createdCategory =
            await createCategoryFromInput(newCategoryName);
      ...
      formData.append(
        "existingImages",
        JSON.stringify(currentEditingProductImages),
      );
      ...
      if ((mainImageFile ? 1 : 0) + galleryFiles.length > 6) {
        showToast(
          "You can upload 1 main image and up to 5 additional images.",
          "error",
        );
        return;
      }
  ...
  window.editProduct = async function (id) {
    ...
    try {
      ...
      if (response.ok) {
        currentEditingProductImages = Array.isArray(product.images)
          ? product.images
          : [];
        selectedGalleryFiles = [];
        if (galleryImageInput) galleryImageInput.value = "";
        ...
        document.getElementById("add-product").querySelector("h1").textContent =
          "Edit Product";
        document
          .getElementById("add-product")
          .querySelector(".btn-primary").textContent = "Update Product";

        // Load categories before switching
        await loadCategories(product.category?._id || product.category);

        const addProductMenu = document.querySelector(
  ...
  document
    .getElementById("download-sales-report")
    ?.addEventListener("click", () =>
      downloadPDF("dashboard", "Sales_Report.pdf"),
    );
  document
    .getElementById("download-inventory-report")
    ?.addEventListener("click", () =>
      downloadPDF("products", "Inventory_Report.pdf"),
    );
  document
    .getElementById("download-tax-report")
    ?.addEventListener("click", () =>
      downloadPDF("dashboard", "Tax_Summary.pdf"),
    );
  ...
  try {
    ...
    if (res.ok) {
      ...
      const revenueEl = document.querySelector(
        "#dashboard .stat-card:nth-child(1) h2",
      );
      const ordersEl = document.querySelector(
        "#dashboard .stat-card:nth-child(2) h2",
      );
      if (revenueEl)
        revenueEl.textContent = `₹${stats.totalRevenue.toLocaleString()}`;
      ...
      if (userRes.ok) {
        const customersEl = document.querySelector(
          "#dashboard .stat-card:nth-child(3) h2",
        );
  ...
  const filtered = feedbackInboxItems.filter((item) => {
    const haystack = [
      ...
      Array.isArray(item.products)
        ? item.products.map((prod) => prod.name).join(" ")
        : "",
    ]
      .join(" ")
      .toLowerCase();
  ...
  chatItems.forEach((chatItem) => {
    chatItem.addEventListener("click", () => {
      ...
      const selected = items.find(
        (item) => item.id === chatItem.dataset.feedbackId,
      );
      if (selected) {
...
function renderFeedbackThread(item) {
  ...
  const productList = Array.isArray(item.products)
    ? item.products
        .map((product) => `${product.name} x${product.qty}`)
        .join(", ")
    : "";
...
function updateDashboardChart(stats) {
  ...
  const labels = stats.monthlyRevenue
    ? stats.monthlyRevenue.map((m) => m.name)
    : ["Jan", "Feb", "Mar", "Apr", "May", "Jun"];
  const data = stats.monthlyRevenue
    ? stats.monthlyRevenue.map((m) => m.value)
    : [0, 0, 0, 0, 0, 0];
...
async function loadRecentOrders() {
  ...
  try {
    ...
    if (res.ok && Array.isArray(orders)) {
      if (orders.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="6" style="text-align:center;color:var(--text-secondary);">No orders yet</td></tr>';
        return;
      }
      let html = "";
      orders
...
async function loadAllOrders() {
  ...
  try {
    ...
    if (tbody)
      tbody.innerHTML =
        '<tr><td colspan="7" style="text-align:center;color:#ef4444;">Failed to load orders</td></tr>';
  }
}
  ...
  try {
    ...
    if (res.ok && Array.isArray(products)) {
      if (products.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="6" style="text-align:center;color:var(--text-secondary);">No products yet. Add your first product!</td></tr>';
        return;
      }
      let html = "";
      products.reverse().forEach((prod) => {
        ...
        const statusStyle =
          stockCount > 0 ? "" : "color:#ef4444;background:rgba(239,68,68,0.1);";
...
async function loadCustomers() {
  ...
  try {
    ...
    if (res.ok && Array.isArray(users)) {
      if (users.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="6" style="text-align:center;color:var(--text-secondary);">No customers yet</td></tr>';
        return;
      }
      let html = "";
...
async function loadAnalytics() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (revenueEl)
        revenueEl.textContent = `₹${stats.totalRevenue?.toLocaleString() || 0}`;
      if (ordersEl)
        ordersEl.textContent = stats.totalOrders?.toLocaleString() || 0;
      if (productsEl && Array.isArray(products))
      ...
      data: {
        labels:
          stats.categorySales.length > 0
            ? stats.categorySales.map((c) => c.name)
            : ["No Data"],
        datasets: [
          {
            label: "Sales (₹)",
            data:
              stats.categorySales.length > 0
                ? stats.categorySales.map((c) => c.value)
                : [0],
